"""
Export BMOLM to ONNX format for browser inference via onnxruntime-web.
...
sys.path.insert(0, os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "bmolm"))
...
def export_onnx(checkpoint_path, tokenizer_path, output_path, quantize=True, push=False):
    ...
    if os.path.exists(config_json):
        ...
        config = BMOConfig(**{k: v for k, v in ckpt["config"].items() if k in valid_fields})
    else:
        config = BMOConfig()
    ...
    print(f"BMOLM: {total:,} params ({total/1e6:.1f}M)")
...
def main():
    parser = argparse.ArgumentParser(description="Export BMOLM to ONNX")
